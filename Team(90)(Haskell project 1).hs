...
putspace1 :: String -> String
putspace1 []=[]
putspace1 [x]=[x]
putspace1 (x:xs)= if elem (first xs) punct then putspace2 (x:xs) else (x:putspace1 xs)

putspace2 :: String -> String
putspace2 []=[]
putspace2 (x:xs)= (x:(' ': (putspace1 xs)))  

wordToken:: String -> [String]
wordToken x= words (putspace1 x)
...
uniqueBigrams []=[]
uniqueBigrams [x]=[]
uniqueBigrams (x:(y:ys))= ((x,y):uniqueBigrams (y:ys))

uniqueTrigrams :: [String] -> [(String,String,String)]
uniqueTrigrams []=[]
uniqueTrigrams [x]=[]
uniqueTrigrams [x,y]=[]
uniqueTrigrams (x:(y:(z:zs)))=((x,y,z):uniqueTrigrams (y:(z:zs)))


pre_bigramsFreq a (x:xs)= numTimesFound (giveme a (x:xs)) (x:xs)

bigo a (x:xs) r = if a>=r then [] else ((giveme a (x:xs),pre_bigramsFreq a (x:xs)):bigo (a+1) (x:xs) r)

change_forms []=[]
change_forms ((x,y):xs)= ((x,y,1):change_forms xs)

numTimesFound _ [] = 0
numTimesFound x list = sum $ map (\a -> 1) $ filter (== x) list

giveme a (x:xs)=(x:xs)!!a

remove_repitions []=[]
remove_repitions [x]=[x]
remove_repitions (x:xs)= if elem x xs then remove_repitions xs else (x:remove_repitions xs)

bingo (x:xs) l= bigo 0 (x:xs) l

bingo1 (x:xs)= bingo (x:xs) (length (x:xs))

bigramsFreq :: Num a => [String] -> [((String,String),a)]
bigramsFreq xs= remove_repitions (bingo1 (uniqueBigrams xs))

trigramsFreq:: Num a => [String] -> [((String,String,String),a)]
trigramsFreq xs= remove_repitions (bingo1 (uniqueTrigrams xs))

getFreq :: (Eq a, Num b) => a -> [(a,b)] -> b
getFreq a []=0
getFreq a ((x,y):xs)= if a==x then y else getFreq a xs


generateOneProb :: Fractional a => ((String,String,String),a) ->[((String,String),a)] -> a
generateOneProb ((w,y,z),a) (x:xs) = if (getFreq (w,y) (x:xs))==0 then 0 else a/(getFreq (w,y) (x:xs))

genProbPairs :: Fractional a => [((String,String,String),a)] ->[((String,String),a)] -> [((String,String,String),a)]
genProbPairs [] x=[]
genProbPairs ((d,a):ys) (x:xs) = ((d,generateOneProb (d,a) (x:xs)):genProbPairs ys (x:xs))

randomelem (x:xs)= (x:xs)!! randomZeroToX (length (x:xs)-1)

powerful_filter [a,b] []=[]
powerful_filter [a,b] (((w,x,y),z):xs)= if w==a && b==x && z>0.03 then (((w,x,y),z):powerful_filter [a,b] xs) else powerful_filter [a,b] xs

giveit ((a,b,c),d)=c

generateNextWord [a,b] (x:xs) =  if length (powerful_filter [a,b] (x:xs))==0 then error "Sorry, it is not possible to infer from current database" else giveit (randomelem (powerful_filter [a,b] (x:xs)))

didi1 = genProbPairs (trigramsFreq(wordTokenList docs)) (bigramsFreq(wordTokenList docs))

try1 str = generateNextWord (getLast2 (wordToken str)) didi1
getLast2 (x:xs)= if length (x:xs)==2 then (x:xs) else getLast2 xs

generateText :: String -> Int -> String
generateText str a= if a==1 then str ++ " " ++ try1 str else generateText (str ++ " " ++ try1 str) (a-1)
...
occurs a []=[] 
occurs a (x:xs) = if occursIn a x then (x:occurs a xs) else occurs a xs
